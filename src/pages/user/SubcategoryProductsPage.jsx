...
const SubcategoryProductsPage = () => {
  ...
  const resolveSubcategory = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/subcategory?category=${encodeURIComponent(cat._id)}&status=Active&page=1&limit=200`);
      const data = await res.json();
      const arr = Array.isArray(data?.data)
        ...
        ? data.subcategories
        : [];
      const match = arr.find((s) => slugify(s.name) === String(subcategorySlugOrId));
      return match || null;
    } catch (e) {
  ...
  if (loading && !products?.length) return <div className="flex justify-center py-16"><Loader message="Loading products..." /></div>;
    ...
    <div className="min-h-screen bg-gray-900 p-4 sm:p-6 lg:p-8 mt-20">
      ...
      <nav className="text-sm text-gray-400 mb-3" aria-label="Breadcrumb">
        <ol className="flex items-center gap-2">
          ...
          <li>/</li>
          <li><Link to={`/c/${categorySlugOrId}`} className="hover:text-white">Subcategories</Link></li>
          <li>/</li>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-white">{subcategory?.name || "Products"}</h1>
        <div className="flex gap-4 text-sm">
          <Link to={`/c/${categorySlugOrId}`} className="text-blue-400 hover:text-blue-300">&larr; Back to Subcategories</Link>
          <Link to="/categories" className="text-blue-400 hover:text-blue-300">Back to Categories</Link>
        </div>
      </div>
      ...
      {(!products || products.length === 0) ? (
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
          {products.map((product) => {
            const raw = product?.images?.[0] || product?.image || product?.thumbnail || product?.mainImage;
            let imgUrl = typeof raw === 'string' ? raw : raw?.url || raw?.path || raw?.src || '';
            if (imgUrl && !/^https?:\/\//i.test(imgUrl)) {
              imgUrl = `${String(API_URL || '').replace(/\/$/, '')}/${String(imgUrl).replace(/^\//, '')}`;
            }
            return (
              <div key={product._id} className="bg-gray-800 rounded-lg overflow-hidden hover:bg-gray-750 transition-colors duration-200 border border-gray-700 hover:border-gray-600">
                ...
                <div className="p-3 sm:p-4">
                  <div className="mb-2">
                    <h3 className="font-semibold text-white text-base sm:text-lg leading-tight line-clamp-2 min-h-[2.5rem]">{product.name}</h3>
                  </div>
                  <div className="flex justify-between items-center mt-4">
                    <div className="text-white font-bold text-base sm:text-lg flex items-center gap-2">
                      ...
                      {product?.originalPrice && product?.discountPrice && (
                        <span className="text-gray-400 line-through text-sm font-normal">${product.originalPrice}</span>
                      )}
                    </div>
                    <Link to={`/product/${product._id}`} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 sm:px-4 rounded text-xs sm:text-sm font-medium transition-colors duration-200">
                      View Details
                    </Link>
                  </div>
                </div>
